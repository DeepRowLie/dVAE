...
class DataLoader:
        ...
        with Parallel(n_jobs=self.n_workers, batch_size="auto", backend="threading") as parallel:
            while start or self.infinite_loop:
                ...
                for minibatch_idx in indices:
                    ...
                    batch_origin = np.concatenate([batch_elem[2] for batch_elem in batch], axis=0)

                    if self.shuffle:
                        self.queue.put((minibatch_idx, batch_input, batch_target, batch_origin))
                    else:
                        self.queue.put((batch_input, batch_target, batch_origin))

                    # Free memory
                    del batch_input, batch_target, batch_origin, batch
    ...
    def _make_batch_element(cls, image_path, augmenter=None):
        ...
        origin_img = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
        origin_img = preprocess_input(origin_img.astype(np.float32), mode="rl")
        ...
        if postprocessor.flipped:
            ...
            target_img = preprocess_image(im, convert_to_rgb=True)
            target_img = target_img.reshape((1,) + target_img.shape)
            if augmenter is None:
                input_img = target_img.copy()
        return input_img, target_img, origin_img
